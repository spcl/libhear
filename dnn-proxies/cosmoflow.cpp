...
int allreduce_sizes[NUM_L-2] = {1050740, 3539456, 884992, 221312, 55360, 3488}; // made last layer to be divisible by 4
...
int run_model_data_parallel(float** fwd_halo_send_buff0_ptrs,
	...
	if(i == NUM_Dense_L-1){
	  ...
	  MPI_Allreduce(grad_ptrs[0], sum_grad_ptrs[0], allreduce_sizes[0], MPI_FLOAT, MPI_SUM, dense_allreduce_comm);
	}
	  ...
	  MPI_Allreduce(grad_ptrs[i-NUM_Dense_L+1], sum_grad_ptrs[i-NUM_Dense_L+1], allreduce_sizes[i-NUM_Dense_L+1], MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD);
	}
    }
...
int main(int argc, char *argv[]){
    ...
    for(int i=0; i<NUM_Conv_L-1; i++){
        fwd_halo_send_buff0_ptrs[i] = (float *)_mm_malloc(conv_fwd_halo_sizes[i] * sizeof(float), 32);
        fwd_halo_send_buff1_ptrs[i] = (float *)_mm_malloc(conv_fwd_halo_sizes[i] * sizeof(float), 32);
        fwd_halo_recv_buff0_ptrs[i] = (float *)_mm_malloc(conv_fwd_halo_sizes[i] * sizeof(float), 32);
        fwd_halo_recv_buff1_ptrs[i] = (float *)_mm_malloc(conv_fwd_halo_sizes[i] * sizeof(float), 32);

        bwd_halo_send_buff0_ptrs[i] = (float *)_mm_malloc(conv_bwd_halo_sizes[i] * sizeof(float), 32);
        bwd_halo_send_buff1_ptrs[i] = (float *)_mm_malloc(conv_bwd_halo_sizes[i] * sizeof(float), 32);
        bwd_halo_recv_buff0_ptrs[i] = (float *)_mm_malloc(conv_bwd_halo_sizes[i] * sizeof(float), 32);
        bwd_halo_recv_buff1_ptrs[i] = (float *)_mm_malloc(conv_bwd_halo_sizes[i] * sizeof(float), 32);
    }
    ...
    for(int i=0; i < NUM_Dense_L; i++){
        dense_fwd_allgather_sbuff_ptrs[i] = (float *)_mm_malloc(dense_fwd_allgather_sizes[i] * sizeof(float), 32);
	dense_fwd_allgather_rbuff_ptrs[i] = (float *)_mm_malloc(dense_fwd_allgather_sizes[i] * model_shards * sizeof(float), 32);
	dense_bwd_rs_sbuff_ptrs[i] = (float *)_mm_malloc(dense_bwd_reduce_scatter_sizes[i] * model_shards * sizeof(float), 32);
	dense_bwd_rs_rbuff_ptrs[i] = (float *)_mm_malloc(dense_bwd_reduce_scatter_sizes[i] * sizeof(float), 32);
    }

    float* grad_ptrs[NUM_L-2];
    float* sum_grad_ptrs[NUM_L-2];
    for(int i=0; i<NUM_L-2; i++){
        grad_ptrs[i] = (float *)_mm_malloc(allreduce_sizes[i] * sizeof(float), 32);
        sum_grad_ptrs[i] = (float *)_mm_malloc(allreduce_sizes[i] * sizeof(float), 32);
    }
