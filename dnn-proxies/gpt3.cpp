...
int main(int argc, char *argv[]){
    ...
    float* grad_ptr = (float *)_mm_malloc(DP_ALLREDUCE_SIZE * sizeof(float), 32);
    float* sum_grad_ptr = (float *)_mm_malloc(DP_ALLREDUCE_SIZE * sizeof(float), 32);

    float* fwd_send_buff = (float *)_mm_malloc(PIPE_P2P_SIZE * sizeof(float), 32);
    float* fwd_recv_buff = (float *)_mm_malloc(PIPE_P2P_SIZE * sizeof(float), 32);
    float* bwd_send_buff = (float *)_mm_malloc(PIPE_P2P_SIZE * sizeof(float), 32);
    float* bwd_recv_buff = (float *)_mm_malloc(PIPE_P2P_SIZE * sizeof(float), 32);
    ...
    for(int i=0; i<2; i++){
	mp_fwd_inter_ptrs[i] = (float *)_mm_malloc(MP_ALLREDUCE_SIZE * sizeof(float), 32);
        sum_mp_fwd_inter_ptrs[i] = (float *)_mm_malloc(MP_ALLREDUCE_SIZE * sizeof(float), 32);
        mp_bwd_grad_ptrs[i] = (float *)_mm_malloc(MP_ALLREDUCE_SIZE * sizeof(float), 32);
        sum_mp_bwd_grad_ptrs[i] = (float *)_mm_malloc(MP_ALLREDUCE_SIZE * sizeof(float), 32);
    }
     
    float* moe_fwd_alltoall_ptrs[2];
    float* moe_bwd_alltoall_ptrs[2];
    for(int i=0; i<2; i++){
        moe_fwd_alltoall_ptrs[i] = (float *)_mm_malloc(MOE_ALL2ALL_SIZE * sizeof(float), 32);
        moe_bwd_alltoall_ptrs[i] = (float *)_mm_malloc(MOE_ALL2ALL_SIZE * sizeof(float), 32);
    }
